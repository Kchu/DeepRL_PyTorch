...
class QUOTA(object):
    ...
    def choose_action(self, x, EPSILON, EPSILON_O ):
        ...
        # 网络输出  (m, N_action, N_quantile), (m, N_option)
        quantile_values, option_values = self.pred_net(x)

        ## 选择Option
        # 获得新的Option(beta)
        greedy_options = torch.argmax(option_values, dim=-1)
        random_options = torch.cuda.FloatTensor(np.random.randint(N_OPTIONS, size=N_ENVS)).long()
        dice = torch.cuda.FloatTensor(np.random.rand(N_ENVS))
        # 根据判断条件决定是random(epsilon)还是greedy(1-epsilon)
        new_options = torch.where(dice < EPSILON_O, random_options, greedy_options)
        # 不采用新的Option(1-beta 保持原来option)
        dice = np.random.rand(N_ENVS)
        # start_new_options = np.logical_or(self.is_initial_states, dice < Behavior_beta)
    ...
    def learn(self):
        ...
        # 预测网络输出 (BATCH_size, N_action, N_quantile), (BATCH_size, N_option)
        ...
        # 目标网络输出 (BATCH_size, N_action, N_quantile), (BATCH_size, N_option)
...
def display_frames_as_gif(frames):
    ...
    anim.save('./quota_breakout_result.gif', writer='imagemagick', fps=30)
...
for t in range(10000):
    ...
    a= quota.choose_action(np.expand_dims(s,axis=0), EPSILON, EPSILON_O)
    # take action and get next state
